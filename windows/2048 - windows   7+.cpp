...
void set_color(WORD attr) {SetConsoleTextAttribute(hOut,attr);}
void reset_color() {SetConsoleTextAttribute(hOut,defaultAttr);}
WORD get_color(uint32_t val){
    if(val==0) return defaultAttr;
    switch(val){
        case 2:case 4:return BACKGROUND_RED|BACKGROUND_GREEN;//黄底黑字
        case 8:case 16:case 32:case 64:case 128:case 256:case 512:case 1024:
        case 2048:return BACKGROUND_RED|BACKGROUND_GREEN|FOREGROUND_RED|FOREGROUND_GREEN|FOREGROUND_BLUE;//黄底白字
        default:return BACKGROUND_RED|BACKGROUND_BLUE|FOREGROUND_RED|FOREGROUND_GREEN|FOREGROUND_BLUE;//紫底白字
    }
}
bool fileok(){
...
#define null_str printf("|"),set_color(get_color(x[i][j])|FOREGROUND_INTENSITY),printf("               "),reset_color()
#define clear_enter printf("|\n")
void print(){
    best=(best<score?score:best);
    set_color(FOREGROUND_RED|FOREGROUND_GREEN|FOREGROUND_BLUE|FOREGROUND_INTENSITY);
    int nlen=getlen(score);system("cls");
    printf("SCORES:%llu%*s",score,27-nlen,"");
    nlen=getlen(best);
    printf("BEST:%llu%*s\n",best,26-nlen,"");
    reset_color();
    fori(i){
        ...
        fori(j){
            if(x[i][j]==0)null_str;
                ...
                nlen=getlen(x[i][j]);printf("|");
                set_color(get_color(x[i][j])|FOREGROUND_INTENSITY);
                printf("%*s%u%*s",(15-nlen)>>1,""/*左空格*/,x[i][j],(16/*16=15+1向上取整*/-nlen)>>1,""/*右空格*/),reset_color();
            }
        }
        clear_enter;
        fori(j)null_str;
